...
export const RELEASES: Release[] = [
  {
    ...
    items: [
      ...
      "Buttons now respond the moment you press them: while the action is running the button dims, locks, and shows a spinner, then flashes green with a tick when it lands — no more pressing Save and wondering whether anything happened. Applied across admin, settings, account and billing forms (the Generate page keeps its own progress display). Locking the button also prevents an impatient double-click from firing the same action twice. Editing the field afterwards resets the button straight back to normal, ready for the next save.",
