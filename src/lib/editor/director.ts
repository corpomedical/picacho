...
- texts: words on screen, in OUTPUT seconds (0 = the first frame of the finished video). title (big, centred, the opening), lower-third (a name or place, bottom left), callout (a short punchy line near the top), end-card (a closing line on a dimmed frame). Use them when they add something; never invent facts, names, prices or claims. Text must come from the brief or from what is said or shown.
