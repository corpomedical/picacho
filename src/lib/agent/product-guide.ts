...
- SETS (Helios 3D): "Helios 3D" under "Tools" in the sidebar (/app/sets), on every paid plan. Describe a location in words and Astra builds it as a real 3D set to keep and reshoot; a photo-based build is not offered to customers. The set page is a workspace with four modes — Build (edit the geometry), Shoot (place the figure and camera exactly; real formats, lenses, stops and light; one credit a still), Film (beats on a timeline, previz free, render takes between a start and an end frame) and Cut (the clips in order; play and download the film as one MP4). On a set's page, the set's own chat (Astra) frames the shot from the person's words — where they stand and face, the camera and the lens, what happens — for free. It never changes the set itself unless the person presses "Change the set" on a card that shows the month's changes left; stills and takes are priced on their buttons; and it shoots on its own only in "Shoot without asking". The Producer can read a set and fix one thing in it as a whole — stand a car back on its wheels, turn it, move it, set it on the floor — free and undoable, saved to the set's Build copy; for anything else it tells the person what to type there. Set builds are capped per month by plan (Basic 1, Starter 2, Growth 5, Studio 10, Elite 25); stills, takes and films are ordinary renders priced in credits and quoted before sending; takes and films are on every paid plan. Free accounts do not have Sets. The guide at /guides/helios explains it.
- FIXING A SET BY HAND (Helios Build, checked against the code 2026-09-25; not on screens narrower than 640 px): open the set and press "Build" in its top bar. Select a block by clicking it on the stage or its row in the Scene list ("Find in the scene…", ⌘K); Esc lets go. The tools on the left rail are Select (V), Move (G, the default), Turn (R) and Size (S), with Camera, Light, Mark and Kit to add things; 1–4 switch the view (Lit, Clay, Wire, Depth). A selected block shows Position, Rotation (degrees, X then Y then Z; type a number and press Enter) and Size; "Snap" (on by default) steps 0.1 m and 15°. "Rest on the ground" sets the selected block on the floor (by its height, not its tilt). ⌘Z / ⇧⌘Z undo and redo; "Astra's original" brings the set back as first built (and can be undone); "Done — Shoot" saves and goes back to Shoot. Build edits ONE BLOCK AT A TIME — a car is about 25 blocks — so turning a whole car over by hand is slow: the Producer turns a whole thing as one, or "Tell Astra what to change" (the prompt bar in Build) rewrites the set in words, which uses one of the month's Astra changes.
- A THING DRAWN FROM A 3D MODEL FILE (admins only for now): the model isn't in Build. In Shoot, tap the thing on the stage to open its card; its "3D MODEL" row has "Build it from its photo", "Load a model file", "Turn it around" (a half turn about the vertical, so it faces the other way — it does not stand an upside-down model on its wheels) and "Back to blocks". The card's "SHAPE" row has "Rebuild from its photos" (with its own Undo). A fix changes the set, not stills already taken: a new still ("Shoot · 1 credit") shows it.
