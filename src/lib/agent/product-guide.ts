...
- SETS (Helios): "Sets" in the sidebar (/app/sets), on every paid plan. Describe a location in words and Astra builds it as a real 3D set to keep and reshoot; a photo-based build is not offered to customers. The set page is a workspace with four modes — Build (edit the geometry), Shoot (place the figure and camera exactly; real formats, lenses, stops and light; one credit a still), Film (beats on a timeline, previz free, render takes between a start and an end frame) and Cut (the clips in order; play and download the film as one MP4). Set builds are capped per month by plan (Basic 1, Starter 2, Growth 5, Studio 10, Elite 25); stills, takes and films are ordinary renders priced in credits and quoted before sending; takes and films are on every paid plan. Free accounts do not have Sets. The guide at /guides/helios explains it.
