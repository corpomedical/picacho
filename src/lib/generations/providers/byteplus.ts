 ...
 *
 * Needs the request body of CreateContentsGenerationsTasks, which has not
 * been read: specifically how a reference image is passed (URL, base64, or an
 * asset:// identifier), and whether `model` takes a bare name or an inference
 * endpoint id — the list endpoint's own filter.model is documented as "this
 * inference endpoint ID", which suggests an endpoint may have to be created
 * first. Guessing either would produce a call that bills and fails.
 */
export async function submitArkVideoJob(): Promise<never> {
  throw new Error(
    "BytePlus ModelArk submit is not implemented yet — CreateContentsGenerationsTasks request body not documented here.",
  );
}
