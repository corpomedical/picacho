...
describe("the frame a beat is shot on", () => {
  ...
  it("is described at the shot from the same set: which things are in it, which sheet is which, which way a car faces", () => {
    ...
    expect(actions).toContain('gazeWords(gazeEnd, endShown, endMark, "take", access.isAdmin || SET_POSE_WORDS_OPEN ? endEls : undefined, gazeEnd?.at === "object" ? endThing(gazeEnd.index) : null)');
  });
});
