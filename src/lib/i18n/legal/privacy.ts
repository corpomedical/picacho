...
// September 26, 2026 is the day Press Tour was disclosed (its own section,
// #press-tour): product cards and ads, the product check's frames going to
// Google (Cloud Vision and the Gemini API), the social accounts a person
// connects to post (encrypted keys, posts only on their press, disconnect
// and deletion), and the apps (Claude, ChatGPT) connected to an account.
// Same day, before it shipped (fixer, PT-R3-08): each post's approval record
...
const privacy: Record<Locale, LegalDoc> = {
  en: {
    ...
    sections: [
      ...
      {
        id: "press-tour",
        heading: "Press Tour: your products, your ads and the accounts you connect",
        paragraphs: [
          "If you use Press Tour, we keep the product cards you make: the product's page address, the photos you pick or upload, the words on its label that you confirm, its logo box and colours, and your answer that you may advertise it. We also keep your brand kit and the ads you make: their plan, stills, filmed shots and finished files.",
          "To read a product and to check each still and each filmed shot against its card, pictures of the product and frames of your ad are sent to Google: Cloud Vision reads the words on a label, and the Gemini API compares the product. They are used only to answer that check. We keep a record of every check (what was read and how it scored, without the picture) to measure how well the check works. The stills and shots themselves are made by our AI generation providers, like Picacho's other pictures and videos.",
          "If you connect a social account (X, TikTok, Instagram or Threads) to post from Press Tour, we keep its name and handle and the keys the network gives us to post for you. The keys are encrypted before they are stored. We use them only to post what you check and send on the press line, when you choose, with the network's own AI label turned on; nothing is ever posted on our own. Each post is kept with a record of exactly what you approved and when, and a scrambled form of the internet address it was sent from. The networks you post to receive the posts you send, and their own privacy policies apply to them from then on.",
          "When you disconnect a social account, we delete its keys, cancel its posts that haven't gone out, and ask X or TikTok to end our access (Instagram and Threads don't offer that; you can also remove Picacho in their settings). Deleting your Picacho account deletes the keys and the records of your posts. If a network doesn't answer when we ask it to end our access, we keep that one key, encrypted, for up to a week only to ask again, then delete it. If you ask us to tell you when posting to a network opens, we keep that and email you once.",
          "If you connect an app such as Claude or ChatGPT to your Picacho account, we keep a record of the connection: which app, what you allowed it to do, and when it was connected and last used. The app gets a key that works only for your account and only for what you allowed; we store only a scrambled form of it. An ad's paid steps start only when you tap Picacho's own card inside the app. You can disconnect an app at any time in Settings › Security › Connected apps, which ends its access at once; deleting your account ends it too.",
        ],
      },
      {
  ...
  es: {
    ...
    sections: [
      ...
      {
        id: "press-tour",
        heading: "Press Tour: tus productos, tus anuncios y las cuentas que conectas",
        paragraphs: [
          "Si usas Press Tour, guardamos las fichas de producto que creas: la dirección de la página del producto, las fotos que eliges o subes, las palabras de su etiqueta que confirmas, el recuadro de su logotipo y sus colores, y tu respuesta de que puedes anunciarlo. También guardamos tu kit de marca y los anuncios que creas: su plan, sus fotos fijas, sus tomas rodadas y sus archivos terminados.",
          "Para leer un producto y comprobar cada foto fija y cada toma rodada con su ficha, se envían a Google imágenes del producto y fotogramas de tu anuncio: Cloud Vision lee las palabras de una etiqueta y la API de Gemini compara el producto. Solo se usan para responder a esa comprobación. Guardamos un registro de cada comprobación (qué se leyó y qué puntuación obtuvo, sin la imagen) para medir lo bien que funciona. Las fotos fijas y las tomas las crean nuestros proveedores de generación con IA, como el resto de imágenes y vídeos de Picacho.",
          "Si conectas una cuenta social (X, TikTok, Instagram o Threads) para publicar desde Press Tour, guardamos su nombre y su usuario, y las claves que la red nos da para publicar por ti. Las claves se cifran antes de guardarse. Solo las usamos para publicar lo que revisas y envías en la rueda de prensa, cuando tú eliges, con la etiqueta de IA de la propia red activada; nunca publicamos nada por nuestra cuenta. Cada publicación se guarda con un registro de exactamente lo que aprobaste y cuándo, y una versión cifrada e irreversible de la dirección de internet desde la que la enviaste. Las redes en las que publicas reciben las publicaciones que envías, y desde ese momento se rigen por sus propias políticas de privacidad.",
          "Cuando desconectas una cuenta social, borramos sus claves, cancelamos sus publicaciones que aún no han salido y pedimos a X o a TikTok que retiren nuestro acceso (Instagram y Threads no lo permiten; también puedes quitar Picacho en sus ajustes). Si eliminas tu cuenta de Picacho, se borran las claves y los registros de tus publicaciones. Si una red no responde cuando le pedimos que retire nuestro acceso, guardamos esa clave, cifrada, hasta una semana solo para volver a pedírselo, y después la borramos. Si nos pides que te avisemos cuando se abra la publicación en una red, lo guardamos y te escribimos un correo una sola vez.",
          "Si conectas una app como Claude o ChatGPT a tu cuenta de Picacho, guardamos un registro de la conexión: qué app, qué le permitiste hacer y cuándo se conectó y se usó por última vez. La app recibe una clave que solo sirve para tu cuenta y solo para lo que permitiste; nosotros solo guardamos una versión cifrada e irreversible de ella. Los pasos de pago de un anuncio solo empiezan cuando tocas la propia tarjeta de Picacho dentro de la app. Puedes desconectar una app cuando quieras en Ajustes › Seguridad › Apps conectadas, lo que termina su acceso al instante; eliminar tu cuenta también lo termina.",
        ],
      },
      {
        id: "facial-information",
  ...
  pt: {
    ...
    sections: [
      ...
      {
        id: "press-tour",
        heading: "Press Tour: seus produtos, seus anúncios e as contas que você conecta",
        paragraphs: [
          "Se você usar o Press Tour, guardamos as fichas de produto que você cria: o endereço da página do produto, as fotos que você escolhe ou envia, as palavras do rótulo que você confirma, a caixa do logotipo e as cores, e sua resposta de que pode anunciá-lo. Também guardamos seu kit de marca e os anúncios que você cria: o plano, as fotos fixas, as cenas filmadas e os arquivos prontos.",
          "Para ler um produto e conferir cada foto fixa e cada cena filmada com a ficha dele, imagens do produto e quadros do seu anúncio são enviados ao Google: o Cloud Vision lê as palavras de um rótulo e a API do Gemini compara o produto. Eles são usados só para responder a essa conferência. Guardamos um registro de cada conferência (o que foi lido e a pontuação, sem a imagem) para medir o quanto ela funciona bem. As fotos fixas e as cenas são criadas pelos nossos provedores de geração com IA, como as outras imagens e vídeos do Picacho.",
          "Se você conectar uma conta social (X, TikTok, Instagram ou Threads) para publicar pelo Press Tour, guardamos o nome e o usuário dela e as chaves que a rede nos dá para publicar por você. As chaves são criptografadas antes de serem guardadas. Só as usamos para publicar o que você confere e envia na coletiva de imprensa, quando você escolher, com o rótulo de IA da própria rede ativado; nunca publicamos nada por conta própria. Cada post é guardado com um registro de exatamente o que você aprovou e quando, e uma versão embaralhada e irreversível do endereço de internet de onde ele foi enviado. As redes em que você publica recebem os posts que você envia, e a partir daí valem as políticas de privacidade delas.",
          "Quando você desconecta uma conta social, apagamos as chaves dela, cancelamos os posts que ainda não saíram e pedimos ao X ou ao TikTok que encerre nosso acesso (Instagram e Threads não oferecem isso; você também pode remover o Picacho nas configurações deles). Excluir sua conta do Picacho apaga as chaves e os registros dos seus posts. Se uma rede não responder quando pedirmos que encerre nosso acesso, guardamos essa chave, criptografada, por até uma semana só para pedir de novo, e depois a apagamos. Se você pedir para avisarmos quando a publicação em uma rede abrir, guardamos isso e mandamos um e-mail uma única vez.",
          "Se você conectar um app como Claude ou ChatGPT à sua conta do Picacho, guardamos um registro da conexão: qual app, o que você permitiu que ele fizesse e quando foi conectado e usado pela última vez. O app recebe uma chave que só funciona para a sua conta e só para o que você permitiu; nós guardamos apenas uma versão embaralhada e irreversível dela. As etapas pagas de um anúncio só começam quando você toca no próprio cartão do Picacho dentro do app. Você pode desconectar um app quando quiser em Configurações › Segurança › Apps conectados, o que encerra o acesso na hora; excluir sua conta também encerra.",
        ],
      },
      {
        id: "facial-information",
  ...
  it: {
    ...
    sections: [
      ...
      {
        id: "press-tour",
        heading: "Press Tour: i tuoi prodotti, i tuoi spot e gli account che colleghi",
        paragraphs: [
          "Se usi Press Tour, conserviamo le schede prodotto che crei: l'indirizzo della pagina del prodotto, le foto che scegli o carichi, le parole dell'etichetta che confermi, il riquadro del logo e i colori, e la tua risposta che puoi pubblicizzarlo. Conserviamo anche il tuo brand kit e gli spot che crei: il piano, le foto di scena, le inquadrature girate e i file finiti.",
          "Per leggere un prodotto e controllare ogni foto di scena e ogni inquadratura girata con la sua scheda, immagini del prodotto e fotogrammi del tuo spot vengono inviati a Google: Cloud Vision legge le parole di un'etichetta e l'API Gemini confronta il prodotto. Vengono usati solo per rispondere a quel controllo. Conserviamo un registro di ogni controllo (cosa è stato letto e con che punteggio, senza l'immagine) per misurare quanto funziona bene. Le foto di scena e le inquadrature sono create dai nostri fornitori di generazione con IA, come le altre immagini e gli altri video di Picacho.",
          "Se colleghi un account social (X, TikTok, Instagram o Threads) per pubblicare da Press Tour, conserviamo il suo nome e il suo nome utente e le chiavi che la rete ci dà per pubblicare per te. Le chiavi vengono cifrate prima di essere conservate. Le usiamo solo per pubblicare ciò che controlli e invii dalla conferenza stampa, quando scegli tu, con l'etichetta IA della rete attiva; non pubblichiamo mai nulla di nostra iniziativa. Ogni post viene conservato con un registro di esattamente ciò che hai approvato e quando, e una versione cifrata e irreversibile dell'indirizzo internet da cui è stato inviato. Le reti su cui pubblichi ricevono i post che invii, e da quel momento valgono le loro informative sulla privacy.",
          "Quando scolleghi un account social, cancelliamo le sue chiavi, annulliamo i post non ancora usciti e chiediamo a X o a TikTok di revocare il nostro accesso (Instagram e Threads non lo prevedono; puoi anche rimuovere Picacho dalle loro impostazioni). Eliminare il tuo account Picacho cancella le chiavi e i registri dei tuoi post. Se una rete non risponde quando le chiediamo di revocare il nostro accesso, conserviamo quella chiave, cifrata, per al massimo una settimana solo per richiederlo, poi la cancelliamo. Se ci chiedi di avvisarti quando apre la pubblicazione su una rete, lo conserviamo e ti scriviamo una sola email.",
          "Se colleghi un'app come Claude o ChatGPT al tuo account Picacho, conserviamo un registro del collegamento: quale app, cosa le hai permesso di fare e quando è stata collegata e usata l'ultima volta. L'app riceve una chiave che funziona solo per il tuo account e solo per ciò che hai permesso; noi ne conserviamo solo una versione cifrata e irreversibile. I passaggi a pagamento di uno spot partono solo quando tocchi la scheda di Picacho dentro l'app. Puoi scollegare un'app quando vuoi in Impostazioni › Sicurezza › App collegate, e il suo accesso finisce subito; anche eliminare il tuo account lo fa finire.",
        ],
      },
      {
        id: "facial-information",
