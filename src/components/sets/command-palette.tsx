    ...
    <div className="fixed inset-0 z-[60] flex items-start justify-center bg-black/50 px-4 pt-[12vh]" onClick={onClose} role="presentation">
      ...
      >
        <input
          ...
          aria-label={words.title}
          className="h-12 w-full border-b border-white/[0.07] bg-transparent px-4 text-[14px] text-[#ecedf1] outline-none placeholder:text-[#6b6f7a]"
        />
        <div ref={listRef} role="listbox" aria-label={words.title} className="max-h-[52vh] overflow-y-auto p-1.5">
          {shown.length === 0 && <p className="px-3 py-4 text-[12.5px] text-[#6b6f7a]">{words.empty}</p>}
          {shown.map((c, i) => (
            <button
              key={c.id}
              type="button"
            ...
            >
              ...
              <span className="whitespace-nowrap text-[10.5px] uppercase tracking-[0.06em] text-[#6b6f7a]">{words.groups[c.group]}</span>
              {c.keys && <kbd className="rounded-[4px] bg-white/[0.06] px-1.5 py-0.5 font-sans text-[10px] font-semibold text-[#9aa0ad]">{c.keys}</kbd>}
            </button>
          ))}
        </div>
        <div className="border-t border-white/[0.07] px-4 py-2 text-[11px] text-[#6b6f7a]">{words.hint}</div>
      </div>
    </div>
  );
}
