...
### THE ARGUMENT WE DID NOT HAVE UNTIL NOW, 2026-09-04

The operator asked the right question: if Eva is fictional, made by our own
image model, shouldn't her photos carry metadata saying so — and shouldn't
their verification read it?

**They do carry it.** Pulled one of Eva's reference images straight out of
character-references and read the bytes. It holds a complete signed C2PA
manifest:

  softwareAgent      name "gpt-image", version c2.0
  digitalSourceType  http://cv.iptc.org/newscodes/digitalsourcetype/
                     trainedAlgorithmicMedia
  signed by          OpenAI OpCo, LLC — "OpenAI Media Service API"
                     with a certificate chain and an "OpenAI TSA Issuing CA"
                     RFC-3161 timestamp
  c2pa.created       2026-08-08

That is the IPTC standard code for "generated by a trained algorithmic model",
cryptographically signed by the generator. And ByteDance refuses the image as
"may contain likenesses of real people".

**So their check is a face detector, not a provenance reader.** It reads
pixels, sees a photorealistic human, and cannot distinguish a person who exists
from one who does not.

**AND THIS REFRAMES THE WHOLE MIGRATION.** Our characters are two different
problems sharing one word:

  * A REAL person needs consent. That is exactly what the verified real-human
    asset library establishes, and it is the right instrument.
  * A SYNTHETIC person has nobody to consent. The asset library's flow — a QR
    code an actor scans, logging into their own BytePlus account to complete
    real-person verification — is meaningless when the actor does not exist.

Eva is the second kind. The asset library, which this file has treated all day
as the unlock, **would not have helped her.** What she needs is for the filter
to read the manifest already embedded in her face.

Worth checking before assuming this generalises: how many of our characters are
generated versus uploaded photographs of real people. That ratio decides which
of the two problems is actually the business.

**Draft ticket (938 chars, under the 1000 cap):**

```
Our characters are AI-generated people, not photographs of real ones, and their reference images carry signed C2PA provenance saying so: softwareAgent gpt-image, digitalSourceType http://cv.iptc.org/newscodes/digitalsourcetype/trainedAlgorithmicMedia, signed by OpenAI OpCo LLC with a certificate chain and an RFC-3161 timestamp.

Seedance 2.0 refuses them anyway — InputImageSensitiveContentDetected.PrivacyInformation on ModelArk, content_policy_violation via resellers.

1. Does the real-person check read C2PA provenance at all? If an image is cryptographically signed as trainedAlgorithmicMedia by a recognised generator, is that considered, or is the check purely pixel-based face detection?

2. If not, is it planned? The real-human asset library solves consent for real actors. It cannot help a person who does not exist — there is nobody to complete verification — so a synthetic character has no route through the filter at all.
```
