...
- **The car changing.** Not fixed. Nothing in a set says which car it is — the description says "a crimson sports coupe" — so each still designs its own. Tested: attaching the first new still as a design reference for a second angle kept the car's design, but also carried over the person's dress, though the prompt said to take nothing else from it. Open: whether continuity should come from an earlier still (strong, but it leaks the person), or from Astra describing its key objects in the spec (weaker, but clean).
