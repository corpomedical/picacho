...
**The look (same day, the operator's choice).** A shot now carries an earlier still from the same set as a design reference, so the car, the furniture and the finishes are the same objects from shot to shot.
- **Which still.** The newest finished still by default; the person can pick another from the contact sheet ("Use its look") or turn it off. The browser names the still by id only. `shootInSet` checks it is a shot of this set, the person's own, finished and not deleted, and that its picture sits in their own folder of generated-images (`look.ts`), then re-signs the path. Anything else and the shot goes without a look.
- **How it rides.** A new attachment role, `look`, only a Set's shot sends. The pipeline passes it as one more extra image after the sketch — only for a still, one character, on GPT Image or FLUX (the models that take an extra image), and only beside a photo of the person. The identity retry carries it too. It is the person's existing picture, not a chat attachment, so deleting the new take never deletes it.
- **What the model is told.** `providers/reference-notes.ts` (where the outfit, attachment and person sentences now live, pinned word for word) names it as an earlier picture of the place whose people are never the identity. Without that sentence it would fall under "every other reference photo is the person". The set's prompt says to keep every object's design, colour, materials and details, and nothing of its camera, framing or light. For the same character, the outfit carries over unless the person's words say otherwise, which is said out loud because the test showed it happens anyway — except when the character has a saved outfit photo, which rides every render and decides the clothes, so the look then promises only the objects (`hasSavedOutfit`, the rule runGeneration applies; the page says which). For another character, the person in it is fenced off. Found in review and fixed before shipping: a look turned off or changed while a still was rendering came back on when the still landed (the page now reads the latest choice), and a saved outfit and a same-character look gave two clothing instructions.
- **Cost.** One more input image per shot: 1,387 → 2,465 input tokens and 196 → 439 output tokens in the test. The person still pays the flat one credit.
...
**The runner is built** (`scripts/astra-sets-eval/`, 2026-09-11; its `README.md` is the operator's guide). Every command is a dry run unless it says `--spend --max-usd <n>`: nothing is called, the plan and its ceiling are printed, and each run ends with `network: 0 live calls, 0 blocked`. A real run reserves every call at its worst case before sending it, stops at the ceiling, and keeps an append-only ledger. It never touches the database, and imports the product's own modules (the model id included) rather than copying them. Built: A, B, D's build leg, the Canary and `report`, which says whether A–D pass at `SET_BUILD_EFFORT`; and the photo arm (`--photos`: A and B on location photos, D on its photos with people; Astra only, never on Batch), which `report` reads apart at `SET_PHOTO_BUILD_EFFORT`; and E, which reads each reference photo 3 times on Astra and on gpt-5.4-mini exactly as Match this shot reads it (never on Batch), holds every read's field of view to the photo's own EXIF and has each read's stage view rated blind. Not built yet: C's engine leg (the stills and their identity scores) and shots carrying a look, and D's stills leg. The corpus template is format-only; the real corpus must be written blind, outside the repo.
