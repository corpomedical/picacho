  ...
  plan(ctx) {
    ...
    return {
      ...
      notes: [
        ...
        `Section 4 estimates E at the one measured read, at standard price. A photo never goes into a Batch input file here (it would sit in OpenAI's Files storage, which the product never does), so every read is at standard price, reserved at its worst case: ${SET_MATCH_INPUT_TOKENS.toLocaleString("en-US")} input tokens at the cache-write rate + ${SET_MATCH_MAX_OUTPUT_TOKENS.toLocaleString("en-US")} output = ${usd(b.astraMatchWorstUsd, 5)} (set-config.ts); ${spec} reads = ${usd(spec * b.astraMatchWorstUsd, 2)}. This is the ceiling; section 4 points here for it.`,
