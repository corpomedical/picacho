...
| C. Stills | Everything: 10 sets × 3 cameras × 2 characters on GPT Image 2 and FLUX.2 (through the product's own `runRealPipeline`) and Seedream v4 edit (the composed route), each still sent as a Set's shot is sent (`lib/shots.mts`): the entry gate, the pipeline's gates, the identity score and gate decision, the tap's prompt-parity check. **The look** (`--look`, off by default): the later cameras shot again carrying camera 1's whole still, which the product stopped sending on 2026-09-12 (it now sends only the set's objects cut out of it: see Known gaps), on the engines the product lets it ride (GPT Image, FLUX); the sheet's objects question, asked of the look shots and their twins alike, and REPORTED lines. The control arm, `c --probe` (with `--look`, the look's request included), the bars in `report`, decided on the shipped effort's runs. If fal refuses the `data:` references, that engine's arm is BLOCKED | The in-app fallback for a BLOCKED FLUX arm (importing an operator's read-only export of in-app Set shots) |
