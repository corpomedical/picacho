...
| C. Stills | Everything: 10 sets × 3 cameras × 2 characters on GPT Image 2.5 and FLUX.2 (through the product's own `runRealPipeline`) and Seedream v4 edit (the composed route), each still sent as a Set's shot is sent (`lib/shots.mts`): the entry gate, the pipeline's gates, the identity score and gate decision, the tap's prompt-parity check. **The look** (`--look`, off by default): the later cameras shot again carrying camera 1's whole still, which the product stopped sending on 2026-09-12 (it now sends an object sheet of the set's objects, drawn from a cutout of it: see Known gaps), on the engines the product lets it ride (GPT Image, FLUX); the sheet's objects question, asked of the look shots and their twins alike, and REPORTED lines. The control arm, `c --probe` (with `--look`, the look's request included), the bars in `report`, decided on the shipped effort's runs. If fal refuses the `data:` references, that engine's arm is BLOCKED | The in-app fallback for a BLOCKED FLUX arm (importing an operator's read-only export of in-app Set shots) |
...
`c --probe` shoots one product fixture set (rainy market) with the first character, on each engine: camera 1 on its own sketch, and with `--look` camera 2 carrying camera 1's whole still as the look on GPT Image and FLUX (the largest request a C run sends: the identity, the sketch and the earlier still). It says whether fal takes the `data:` references (FLUX), with the look too when asked, what size Seedream's square option (`square_hd`, in `lib/seedream.mts`) comes back at, whether the gates and the scorer answer, and which hosts were reached:
...
- **The look arm** (`--look` adds it; off by default, see Known gaps): for each set, character and engine, camera 1's still is shot first; cameras 2 and 3 are shot again carrying it, whole, as the look (the product now sends an object sheet of its objects), with the same character; only on GPT Image and FLUX, the engines a look can ride. 40 more on each. Each look shot and its twin on the same sketch share a direction, so they differ by the look alone.
...
**A still's prompt carries Astra's words.** A Set's shot prompt is the set's description (Astra's), Picacho's framing sentences and, when the person typed one, their direction. Since 2026-09-12 (`refusal-attribution.ts`), when our gate refuses such a prompt, the product judges the same prompt without the direction on its own (the same gate, the same lane and the same session history as the refusal it explains) and logs the refusal by whose words made the difference: under Astra, never counted, when that part is refused alone or there is no direction at all; against the person, counted, when it passes alone, and also when that judgement fails. The runner decides each refused set or look still's prompt the same way, with the product's own `decideRefusalProvider` and the same gate call, asked once, handed the count the refusing gate read: the entry gate's (0, or with `--escalate` the count so far), or 0 for the pipeline's gate and the Seedream route's, which read 0 here (see the known gaps). That is one more gate call, metered, and never sent once the run is stopping. The still records who the refusal is logged against and how that was decided (`attribution`: no direction, judged alone, judgement failed, or not judged when the run stopped first), and each D outcome lists its refused still prompts (`refusedShotPrompts`), whatever else its stills did. The "zero refusals of model-written text counted" bar reads them against the source, which the runner checks at run time (`policy-log.ts`, `pipeline.ts`, `sets/actions.ts`). On the product that judges Astra's part alone it passes by construction, and its arithmetic says how each refused still prompt was logged. It stays UNDETERMINED while a refused still prompt's Astra part went unread on its own: the judgement failed or could not read (the product then counts it without knowing whose words were refused), the run stopped first, or the row comes from a D run made before this, which records no attribution. On the source before 2026-09-12, which logged every refused still prompt with no provider, it FAILS on any of them, with the ids; on a source that shows neither, it asks for a re-verify. `--escalate` counts only the person's, and shoots a brief's stills one at a time, as the product's separate takes are: each still's gate reads every refusal counted before it, the brief's own Astra refusal and its earlier stills' included.
...
| B, B photos | $0 | — |
| C: 10 sets × 3 cameras × 2 characters, 3 engines; controls; the look arm with `--look` | GPT Image (60 set + 20 control) × 2 × $0.17 = $27.20 reserved ($13.60 if each renders once; `--look` adds 40 × 2 × $0.17 = $13.60: $40.80) | FLUX 240 renders reserved, Seedream 60; gates up to 300 entry + 300 pipeline + 300 output, and up to 260 more (60 set × 3 + 40 look × 2) if every set or look still's prompt were refused and judged again without the direction; 300 identity scores; 40 drafts |
| D: 40 briefs × 3 runs, background (standard), with the stills | 120 × $1.155 = $138.60 (`--runs 1`: $46.20), plus each harmful brief run's stills: × `--d-cameras` × 2 × $0.17 | brief gate 120, words gate up to 240; per still 2 prompt gates (and a third if one refuses its prompt: the prompt without the direction, judged alone), the output gate, 1 identity score |
| `c --probe` | 1 still (camera 1) × 2 × $0.17 = $0.34; with `--look` 2 stills (camera 2 carrying camera 1's still too) = $0.68 | FLUX 2 renders (4 with `--look`), Seedream 1; 3 (5) of each gate and score |
...
- Without `policyAudit` the pipeline's own gates read `sessionPriorHits` 0, even where `d --escalate` hands the entry gate a count. A prompt they refuse is judged again without the direction at 0 as well: the count its own gate read, as the product does.
- The Seedream route gates the shot prompt with the pipeline's notes (the product's pipeline gate reads it before they are added), so a prompt refused there is judged again without the direction with those notes too: the same prompt with the person's words taken out, as the product's rule is written. GPT Image and FLUX go through the pipeline's own gate.
- The look: the product's default follows the newest still with a recorded camera (`set-view.tsx`); the eval pins camera 1's, so every look shot is judged against one first still.
- C's look arm still sends camera 1's whole still, where the product sends only the set's objects, cut out of the still onto grey by SAM 2 (`sets/look-cutout-store.ts`): its prompt is already the product's, whose look sentence describes that cutout, so a look shot measures neither the old look nor the new one until the eval cuts the look out as the product does. The arm is off until then; `--look` runs it anyway.
