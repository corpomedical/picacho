...
- C's look arm still sends camera 1's whole still until it follows track 7, where the product sends only the set's objects, cut out of the still onto grey by SAM 2 (`sets/look-cutout-store.ts`): its prompt is already the product's, whose look sentence describes that cutout, so until then a look shot measures neither the old look nor the new one.
