...
| B, B photos | $0 | — |
| C: 10 sets × 3 cameras × 2 characters, 3 engines; the look arm; controls | GPT Image (60 set + 40 look + 20 control) × 2 × $0.17 = $40.80 reserved ($20.40 if each renders once; `--no-look`: $27.20) | FLUX 240 renders reserved, Seedream 60; gates up to 300 entry + 300 pipeline + 300 output, and up to 260 more (60 set × 3 + 40 look × 2) if every set or look still's prompt were refused and judged again without the direction; 300 identity scores; 40 drafts |
...
- C's look arm still sends camera 1's whole still, where the product sends only the set's objects, cut out of the still onto grey by SAM 2 (`sets/look-cutout-store.ts`): its prompt is already the product's, whose look sentence describes that cutout, so a look shot measures neither the old look nor the new one until the eval cuts the look out as the product does. Until then, run C with `--no-look`.
