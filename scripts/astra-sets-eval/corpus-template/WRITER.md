...
- **Only pictures you have the right to send to OpenAI and Anthropic**: your own, or ones whose licence allows it. Say which in `licence`.
...
- **People: consent, or AI-generated people**, exactly as for `people-photos.json`: set `containsPeople` to `true` for any picture in which a person can be seen, and give its `consent`, with `covers` naming both `OpenAI` and `Anthropic`. A person is used only with the consent of everyone recognisable, or when every person in the picture was made by an AI image tool. **Never a photo of real people found or scraped online.**
